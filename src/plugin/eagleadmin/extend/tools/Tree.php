<?php
namespace plugin\eagleadmin\extend\tools;
...
class Tree
{
    ...
    public static function getTreeArray(array $arr, string $field = 'name', int $level = 0, bool $superiorEnd = false): array
    {
        ...
        foreach ($arr as $key => $item) {
            ...
            if ($level == 2) {
                $arr[$key][$field] = Tree . phpstr_pad('', 4) . $item[$field];
            } elseif ($level >= 3) {
                $arr[$key][$field] = Tree . phpstr_pad('', 4) . str_pad('', ($level - 2) * 4) . $prefix . $item[$field];
            }
    ...
    public function getChildrenArray($data=[],$pid=0): array
    {
        ...
        foreach ($data as $item) {
            if ($item["parent_id"]==$pid) {
                $ret[] = $item["id"];
                $ret = array_merge($ret,$this->getChildrenArray($data,$item["id"]));
            }
        }
